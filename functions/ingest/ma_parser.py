...
def ingest_ma_to_bronze(file_content, file_name, portco_id):
    ...
    if any(id_match in portco_id.lower() for id_match in ["alpha", "hotel", "hospitality"]):
        ...
        normalized = []
        for r in alpha_rows:
            typ = r.get("value_type") or r.get("type") or "actual"
            bl = r.get("business_line")
            era = r.get("era")
            # Encode business_line + era into column_label so bronze preserves it
            # without schema changes. Format: "<value_type>|<business_line>|<era>"
            column_label = typ
            if bl:
                column_label = f"{typ}|{bl}"
            if era:
                column_label = f"{column_label}|{era}" if bl else f"{typ}||{era}"
